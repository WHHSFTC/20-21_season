...
import org.firstinspires.ftc.teamcode.geometry.degToRad
...
class SummumLocalizer(val bot: Summum) : DeadWheelLocalizer(bot.logger,
        odos = listOf(
                ...
                bot.config.encoders["rightOdo"] to Pose2d(1.71, -7.375, 180.0.degToRad()),
                bot.config.encoders["backOdo"] to Pose2d(-2.41, -7.375, -90.0.degToRad())
        )
) {
        override fun load() {
        }
